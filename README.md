...
> ⚠️ **Honest disclaimer before you install:**
> - Paywall design is generated by **RevenueCat's built-in AI** (not custom)
> - CVR / conversion data **cannot be fetched automatically** — RC API doesn't expose experiment analytics. You check the RC Dashboard yourself.
> - Starting an experiment **requires clicking Start in RC Dashboard** — RC API has no endpoint for creating experiments.
...
|------|---------|-------------|
| `setup` | "Start a paywall A/B test" | Creates RC Offering via API → RC AI generates Paywall → Asks YOU to create Experiment in RC Dashboard → Registers 3-day cron |
| `check_in` | Every 3 days at 7am (cron) | Sends Slack: "Go check RC Dashboard. Report A wins / B wins / not yet" |
| `analyze` | You reply "A wins" or "B wins" | Fetches offering details → LLM analyzes why winner won → Generates 3 new copy variants → Posts to Slack |
| `create_variant` | You pick variant 1/2/3 | Creates new RC Offering + Paywall (RC AI) → Asks YOU to start new experiment in RC Dashboard |
...
SLACK_METRICS_CHANNEL=C...  ← Channel ID where notifications are posted
```
...
### 5. Pencil MCP (optional — only for `create_variant` mode)
...
Find these IDs:
- `rc_project_id` → RC Dashboard URL: `app.revenuecat.com/projects/YOUR_ID`
...
4. After you provide the experiment ID → registers the 3-day cron

### 3-day loop
Every 3 days at 7am, you get a Slack message:
```
📊 Paywall Experiment Day 3
...
You check the dashboard → reply → the loop continues.
...
paywall-ab-skill/
├── README.md                    ← This file
├── SKILL.md                     ← Main skill (all 4 modes)
├── apps.json.template           ← Configuration template
└── slack-approval/
    ├── SKILL.md                 ← Approval utility skill
    └── scripts/
        ├── approve.js           ← Node.js Block Kit approval
        └── package.json        ← npm: @slack/web-api, @slack/socket-mode
```
